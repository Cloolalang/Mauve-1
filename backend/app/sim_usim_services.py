"""USIM service n° labels aligned with 3GPP TS 31.102 Annex E (EF_UST), Rel ~19 table."""

from __future__ import annotations

# Source: 3GPP TS 31.102 EFUST table (service n°1 …); mirrored on public summaries such as
# https://itecspec.com/3gpp/31.102/s/4.2.8 — numbering differs from older spec revisions.
USIM_SERVICE_LABELS: dict[int, str] = {
    1: "Local Phone Book",
    ...
    24: "Enhanced MultiLevel Precedence and Preemption Service",
    ...
    33: "Reserved (shall be set to 1 if byte present)",
    ...
    113: "URI support for SMS-PP DOWNLOAD (TS 31.111)",
    114: "From Preferred",
    ...
    148: 'UE configured for using "Operator controlled signal threshold per access technology"',
    149: "A2X",
    150: "IMS Data Channel Indication",
    151: "Operator-controlled-LSP-PLMN",
    152: "PeriodicSearchTimerNonLSP",
}


def label_usim_service(service_no: int) -> str:
    """Human-readable label for USIM service n°, or generic fallback."""
    n = int(service_no)
    if n < 1:
        return "Invalid service number"
    if n in USIM_SERVICE_LABELS:
        return USIM_SERVICE_LABELS[n]
    return f"USIM service n°{n} (see 3GPP TS 31.102 Annex E — may be defined in a newer release)"


SIM_EF_DESCRIPTIONS: dict[str, str] = {
    "ef_plmnwact": "User-controlled preferred PLMN list with access technology (EF_PLMNwAcT).",
    "ef_oplmnwact": "Operator-controlled preferred PLMN list with access technology (EF_OPLMNwAcT).",
    "ef_ehplmn": "Equivalent HPLMN list (EF_EHPLMN).",
    "ef_fplmn": "Forbidden PLMN list (EF_FPLMN).",
    "ef_ad": "Administrative data (EF_AD), including MNC length for IMSI-derived PLMN parsing.",
    "ef_hplmn": "Higher priority PLMN search period timer (EF_HPPLMN / EF_HPLMN timer field).",
    "ef_spdi": "Service Provider Display Information (EF_SPDI).",
    "ef_ust": "USIM Service Table (EF_UST): one bit per optional service (TS 31.102 Annex E).",
    "ef_pnn": "PLMN Network Name records (EF_PNN).",
    "ef_opl": "Operator PLMN List (EF_OPL).",
    "ef_epsloci": "EPS location information on the USIM (EF_EPSLOCI; GUTI/TAI-related subscription context).",
    "ef_5gsloci": "5GS location / mobility subscription data (EF_5GSLOCI / EF_5GS3GNSI etc., module-dependent).",
}


SIM_INSPECTOR_LABEL_REFERENCE = "3GPP TS 31.102 EFUST (USIM Service Table), Annex E / §4.2.8 (Rel 19 numbering)"
